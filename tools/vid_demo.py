...
from yolox.utils import fuse_model, get_model_info, postprocess, vis, vis_with_mask
import random
...
class Predictor(object):
    def __init__(
        ...
        self.experiment_name = exp.exp_name
    ...
    def inference(self, img,img_path=None):
        ...
        with torch.no_grad():
            t0 = time.time()
            # CONDITION ADDED TO PARSE MASK AS WELL
            if "inf" in self.experiment_name:
                outputs, outputs_ori, masks = self.model(img, nms_thresh = self.nmsthre)  # FOR SAM
                # print("First image prediction Mask shape :", masks[0].shape)
...
class PredictorWithMask(Predictor):
    ...
    def visual(self, output,img,ratio, cls_conf=0.0, mask=None):
        ...
        if mask is not None:
            # Assuming masks is a list of tensors where each tensor is [N, 28, 28]
            # and we're picking the right one for this image.
            mask = mask.cpu().numpy()
            # print("mask shape in visual", mask.shape)

        vis_res = vis_with_mask(img, bboxes, scores, cls, cls_conf, self.cls_names, masks=mask)
        return vis_res
...
def imageflow_demo_mask(predictor, vis_folder, current_time, args):
    gframe = args.gframe
    cap = cv2.VideoCapture(args.path)
    width = cap.get(cv2.CAP_PROP_FRAME_WIDTH)  # float
    height = cap.get(cv2.CAP_PROP_FRAME_HEIGHT)  # float
    ...
    for ele in res:
        ...
        ele = torch.stack(ele)
        t0 = time.time()
        output, mask = predictor.inference(ele)
        outputs.extend(output)
        mask_outputs.extend(mask)

    outputs = [j for _,j in sorted(zip(index_list,outputs))]
    for output,img,mask_output in zip(outputs,ori_frames[:len(outputs)], mask_outputs):
        result_frame = predictor.visual(output,img,ratio,cls_conf=args.conf, mask=mask_output)
        if args.save_result:
            vid_writer.write(result_frame)
...
def main(exp, args):
    ...
    if args.dataset=='vid':
        # CONDITION ADDED TO PARSE MASK AS WELL
        if "inf" in args.experiment_name:# FOR SAM
            predictor = PredictorWithMask(model, exp, VID_classes, trt_file, decoder, args.device, args.legacy)
        else:
    ...
    if "inf" in args.experiment_name:  # FOR SAM
        imageflow_demo_mask(predictor, vis_folder, current_time, args)
    else:
