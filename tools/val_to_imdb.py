...
class Predictor(object):
    ...
    def inference(self, img,img_path=None):
        ...
        with torch.no_grad():
            t0 = time.time()
            if "sam" in self.experiment_name:
                outputs, outputs_ori = self.model(img)  # FOR SAM
            elif "_vid" in self.experiment_name:
                outputs = self.model(img)  # FOR YOLOX Single Frame Baseline
            else:
                outputs,outputs_ori = self.model(img)  # DEFAULT YOLOV

        if "_vid" not in self.experiment_name and len(outputs)<=4:
            outputs = outputs_ori
