...
from yolox.core.vid_trainer import Trainer, TrainerVOWSAM
...
def make_parser():
    ...
    parser.add_argument(
        ...
        default=False,
        action="store_true",
        help="Loading data from Zip files for fast training, only mention if zip this feature is required",
    )
    parser.add_argument(
        "--opts",
...
def main(exp, args):
    ...
    if "sam" in exp.exp_name:
        trainer = TrainerVOWSAM(exp, args, val_loader, val=False)
    else:
...
if __name__ == "__main__":
    ...
    exp.merge(args.opts) if args.opts else None
