...
class VIDEvaluator:
    ...
    def __init__(
        self, dataloader, img_size, confthre, nmsthre, num_classes, testdev=False, gl_mode = False, experiment_name=""
    ):
        """
        Args:
        ...
        for cur_iter, (imgs, _, info_imgs,label,path,time_embedding) in enumerate(
            ...
            with torch.no_grad():
                ...
                if "inf" in self.experiment_name:
                    outputs, ori_res, mask_output = model(imgs) #FOR SAM
                else:
                    outputs,ori_res = model(imgs) #DEFAULT YOLOV
