...
# from yolox.data.datasets.vid import DataPrefetcher as DataPrefetcher
from yolox.data.datasets.vid import DataPrefetcherSAM as DataPrefetcher
...
class Trainer:
    ...
    def train_one_iter(self):
        ...
        with torch.cuda.amp.autocast(enabled=self.amp_training):
            ...
            # print(f"OUTPUTS : after FORWARD PASS : {outputs}")
            ...
            if self.args.logger == "tensorboard":
                ...
                for k, v in zip(self.args.opts[0::2], self.args.opts[1::2]):
                    if k.startswith("wandb-"):
                        wandb_params.update({k.replace("wandb-",""): v})
...
class TrainerVOWSAM(Trainer):
    def __init__(self, exp, args, val_loader, val=False):
        super().__init__(exp, args, val_loader, val)
        # add any additional initialization here

    def train_one_iter(self):
        '''
        - Overriding from parent class.
        - Introducing mask targets for VOWSAM
        Returns:
        ...
        #instances per image is added to track no. of instance mask during loss computation
        inps, targets, targets_mask, mask_confidence_scores = self.prefetcher.next()

        inps = inps.to(self.data_type)
        targets = targets.to(self.data_type)
        targets_mask = targets_mask.to(self.data_type)
        # mask_confidence_scores  = mask_confidence_scores.to(self.data_type)
        targets.requires_grad = False
        targets_mask.requires_grad = False
        # mask_confidence_scores.requires_grad = False

        inps, targets, targets_mask = self.exp.preprocess(inps, targets, targets_mask, self.input_size)
        data_end_time = time.time()

        with torch.cuda.amp.autocast(enabled=self.amp_training):
            # print(targets_mask)
            outputs = self.model(inps, targets, target_masks=targets_mask, mask_confidence_score=mask_confidence_scores)
    ...
    def after_epoch(self):
        '''
        Modification:
        - Saving each epoch, instead of just the best one based on mAP_50
        '''
        epoch_num= self.epoch+1
        self.save_ckpt(ckpt_name="epoch_{}".format(epoch_num))
