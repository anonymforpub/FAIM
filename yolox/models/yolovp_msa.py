...
class YOLOXHead(nn.Module):
    def __init__(
        ...
        self.cls_convs2 = nn.ModuleList()#ADDED BY THE YOLOV
    ...
    def forward(self, xin, labels=None, imgs=None, nms_thresh=0.5, masks=None):
        outputs = []
        outputs_decode = []
        origin_preds = []
        x_shifts = []
        ...
        for k, (cls_conv, cls_conv2, reg_conv, stride_this_level, x) in enumerate(
            ...
            if self.training:
                ...
                #shifts contain x and y coordinates for each anchor location
                # on the feature map. They used to map anchor boxes back to orignal
                # image space.
        ...
        #making output relative to the entire image instead of each cell in feature maps
        decode_res = self.decode_outputs(outputs_decode, dtype=xin[0].type())

        #final preds after NMS and filtered top K predictions for each image
        ...
        #extracting the classification and regression features, classification and objectness scores
        # for the top simN (Here simN=30) bounding box predictions for each image in the batch
    ...
    def get_losses(
        ...
        # calculate targets
        #if mixup, labels would have >5 columns
