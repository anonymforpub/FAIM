...
# from mmtrack.models import SELSA
...
# def get_model_info(model: nn.Module, tsize: Sequence[int],data) -> str:
#     print('Batch Size is : ', data['img'][0].size(0))
#     print(data['img'][0].shape)
#     img = torch.zeros((2, 3, 576, 1008))
#     analysis_results = get_model_complexity_info(model, (2, 3, 576, 1008))
...
#     if all_on_cuda:
#         print("All parameters are on cuda:0")
#     else:
#         print("Not all parameters are on cuda:0")

#     # Check if all buffers are on cuda:0
#     all_buffers_on_cuda = True
#     for name, buffer in model.named_buffers():
#         # print(f"{name} is on {buffer.device}")
#         if buffer.device.type != 'cuda' or buffer.device.index != 0:
#             all_buffers_on_cuda = False

#     if all_buffers_on_cuda:
#         print("All buffers are on cuda:0")
#     else:
#         print("Not all buffers are on cuda:0")

#     stride = 640
#     # img = torch.zeros((2, 3, 576, 1008)).cuda()
#     original_model = model.module if isinstance(model, torch.nn.DataParallel) else model
#     parameters_total= get_number_of_parameters(original_model)
#     # print('total_paramts:', parameters_total)
#     # exit()
#     print((type(data)))
#     print(data.keys())
#     inputs_data = (data['img'], data['ref_img'])

#     # inputs_data = (data['img'], data['annotations']['gt_bboxes'], data['gt_labels'], data['ref_gt_bboxes'], data['ref_gt_labels'])
#     # # exit()
#     flops, params = profile(original_model, inputs= inputs_data, verbose=True)
#     exit()
#     # flops, params = profile(model, inputs=img, verbose=True)
#     # flops, params = profile((model), inputs=(.to('cuda')), verbose=True)
#     params /= 1e6
#     flops /= 1e9
#     flops *= tsize[0] * tsize[1] / stride / stride * 2  # Gflops
#     info = "Params: {:.8f}M, Gflops: {:.8f}".format(params, flops)
#     # exit()
#     return info
...
def single_gpu_test(model,
    ...
    for i, data in enumerate(data_loader):
        ...
        print('\n result keys are :',result.keys())
        print('\n results plural keys are :',results.keys())
        print('filename',data['img_metas'][0].data[0][0])
